<x-app-layout>
    ...
    <div class="mb-8">
        ...
        <p class="text-sm text-gray-500 mt-1">Olá, {{ auth()->user()->name }}! Aqui está um resumo dos chamados.</p>
    </div>

    <!-- Cards de stats -->
    <div class="grid grid-cols-2 lg:grid-cols-5 gap-4 mb-8">
        <a href="{{ route('tickets.index') }}" class="bg-white rounded-xl border border-gray-100 p-5 hover:shadow-md transition-shadow group">
            ...
            <div class="text-xs text-gray-400 mt-1">chamados</div>
        </a>

        <a href="{{ route('tickets.index', ['status' => 'aberto']) }}" class="bg-white rounded-xl border border-blue-100 p-5 hover:shadow-md transition-shadow group">
            <div class="flex items-center justify-between mb-3">
        ...
        <a href="{{ route('tickets.index', ['status' => 'em_andamento']) }}" class="bg-white rounded-xl border border-yellow-100 p-5 hover:shadow-md transition-shadow group">
            <div class="flex items-center justify-between mb-3">
        ...
        <a href="{{ route('tickets.index', ['status' => 'resolvido']) }}" class="bg-white rounded-xl border border-green-100 p-5 hover:shadow-md transition-shadow group">
            <div class="flex items-center justify-between mb-3">
        ...
        <a href="{{ route('tickets.index', ['priority' => 'critica']) }}" class="bg-white rounded-xl border border-red-100 p-5 hover:shadow-md transition-shadow group">
            <div class="flex items-center justify-between mb-3">
    ...
    <div class="bg-white rounded-xl border border-gray-100 overflow-hidden">
        <div class="flex items-center justify-between px-6 py-4 border-b border-gray-100">
            <h2 class="font-semibold text-gray-900">Chamados Recentes</h2>
        ...
        @if ($recentTickets->isEmpty())
            <div class="py-16 text-center">
                ...
                <p class="text-gray-400 font-medium">Nenhum chamado ainda</p>
                <p class="text-gray-300 text-sm mt-1">Crie o primeiro chamado para começar</p>
                ...
                <table class="w-full text-sm">
                    <thead>
                        <tr class="border-b border-gray-50 bg-gray-50/50">
                            <th class="text-left px-6 py-3 text-xs font-medium text-gray-500 uppercase tracking-wide">Número</th>
                            <th class="text-left px-6 py-3 text-xs font-medium text-gray-500 uppercase tracking-wide">Título</th>
                            <th class="text-left px-6 py-3 text-xs font-medium text-gray-500 uppercase tracking-wide">Status</th>
                            <th class="text-left px-6 py-3 text-xs font-medium text-gray-500 uppercase tracking-wide">Prioridade</th>
                            <th class="text-left px-6 py-3 text-xs font-medium text-gray-500 uppercase tracking-wide hidden md:table-cell">Solicitante</th>
                            <th class="text-left px-6 py-3 text-xs font-medium text-gray-500 uppercase tracking-wide hidden lg:table-cell">Data</th>
                        </tr>
                    </thead>
                    <tbody class="divide-y divide-gray-50">
                        @foreach ($recentTickets as $ticket)
                            <tr class="hover:bg-gray-50/50 transition-colors">
                                <td class="px-6 py-4">
                                    <a href="{{ route('tickets.show', $ticket) }}" class="font-mono text-xs text-indigo-600 hover:text-indigo-800 font-medium">
                                        {{ $ticket->number }}
                                    </a>
                                </td>
                                <td class="px-6 py-4 max-w-xs">
                                    <a href="{{ route('tickets.show', $ticket) }}" class="text-gray-900 hover:text-indigo-600 font-medium truncate block">
                                        {{ $ticket->title }}
                                    </a>
                                    @if ($ticket->category)
                                ...
                                <td class="px-6 py-4 text-gray-500 hidden md:table-cell">{{ $ticket->requester?->name }}</td>
                                <td class="px-6 py-4 text-gray-400 text-xs hidden lg:table-cell">{{ $ticket->created_at->diffForHumans() }}</td>
                            </tr>
                        @endforeach
                    </tbody>
                </table>
